Shader "AkilliMum/Standard/D2WeatherEffects/Sprite/D2SnowsExt"
{
    Properties
    {
        ...
        [HideInInspector]LAYERSSTART("Layers Start", float) = 0
        [HideInInspector]LAYERSEND("Layers End", float) = 50
            //[HideInInspector]_Size("Size", float) = 0.1
            [HideInInspector]SPEED("Speed", float) = 1
            [HideInInspector]DEPTH("Depth", float) = 1.2
            [HideInInspector]WIDTH("Direction", float) = 1
    ...
    SubShader
    {
        ...
        float calcSnow(float2 uv)
        {
            ...
            for (int i = LAYERSSTART; i < LAYERSEND; i++) {
                float fi = float(i);
                float2 q = uv * (1. + fi * DEPTH);
                q += float2 (q.y * (WIDTH * mod(fi * 7.238917, 1.) - WIDTH * .5), SPEED * _Time.y / (1. + fi * DEPTH * .03));
                ...
                float res = smoothstep(edge, -edge, d) * (r.x / (1. + .02 * fi * DEPTH));
                acc += float3 (res, res, res);
            }
            return acc;
        ...
        void surf(Input IN, inout SurfaceOutput o)
        {
            ...
            //o.Albedo = (1 - m) * /*_Color.rgb * */ _Luminance;
            ///*Albedo.x = min(1, Albedo.x);
            //Albedo.y = min(1, Albedo.y);
            //Albedo.z = min(1, Albedo.z);*/
            //o.Alpha = top * right * bottom * left * m * _Luminance;// *Mask;
            ////Color = Color * float4(Albedo, Alpha);

            o.Albedo = top * right * bottom * left * m * _Color.rgb * _Luminance;
            o.Alpha =  top * right * bottom * left * m * _Luminance;
