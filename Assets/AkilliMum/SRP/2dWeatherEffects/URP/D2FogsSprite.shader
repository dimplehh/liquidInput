...
Shader "AkilliMum/SRP/D2WeatherEffects/URP/D2Fogs"
{
    Properties
    {
        ...
        [HideInInspector]_CurveTex("Curve", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //my pragmas
            #pragma shader_feature _USECURVE
...
// 				o.clipPos = vertexInput.positionCS;
            ...
            float texCurve(float2 uv) {
                return SAMPLE_TEXTURE2D(_CurveTex, sampler_CurveTex, uv).r;
            }

            float fog(in float2 uv, in float2 texUv)
            {
                float curveMultiplier = 1;
#ifdef _USECURVE
                //float onePixelX = (1.0 / (_ScreenParams.xy / _ScreenParams.w).x);
                //float onePixelY = (1.0 / (_ScreenParams.xy / _ScreenParams.w).y);

                float2 texcoord1 = float2(texUv.x - 0.1, 0.0); //just sample the x
                float2 texcoord2 = float2(texUv.x + 0.1, 0.0); //just sample the x
                float curveLeft  = texCurve(texcoord1);
                float curveRight = texCurve(texcoord2);

                curveMultiplier = curveLeft - curveRight;

                //curveMultiplier = texCurve(texcoord);

#endif

                float direction = _Time.y * _Speed;
                //float Vdirection = _Time.y * _VSpeed;
                float Vdirection = _Time.y * _VSpeed * curveMultiplier;
                float color = 0.0;
                float total = 0.0;
                float k = 0.0;
            ...
            half4 CombinedShapeLightFragment(Varyings i) : SV_Target
            {
                ...
                half4 Color = CombinedShapeLightShared(main, tex, i.lightingUV);
#else
                ...
                float Alpha =  top * right * bottom * left * m * Mask;
