...
Shader "AkilliMum/SRP/D2WeatherEffects/URP/D2Rains"
{
    Properties
    {
...
// 				o.clipPos = vertexInput.positionCS;
            ...
            float calcSnow(float2 uv)
            {
                ...
                for (float i=0.; i < _Multiplier; i++)
                {
                  ...
                  //q += float2(q.y*w, _Speed*_Time.y / (1.0+_Zoom*0.03));
                    ...
                    snow += smoothstep(0.03,-0.03,d)*(r.x/(1.+.02*i*_Zoom));
                }
                return snow;
            }
            ...
            half4 CombinedShapeLightFragment(Varyings i) : SV_Target
            {
                ...
                //float3 Albedo = top * right * bottom * left * (1-m) * _Color.rgb; // * _DarkMultiplier;
                float3 Albedo = (1 - m) * _Color.rgb * _Luminance;
                float Alpha =  top * right * bottom * left * m * Mask; // * _DarkMultiplier;
