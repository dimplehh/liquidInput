...
public class Player : MonoBehaviour
{
    ...
    int maxWalkSoundCnt = 3;
    int curWalkSoundCnt = 0;
    private void Awake()
    {
    ...
    private void SoundSelect()
    {
        ...
        if (curWalkSoundCnt++ == maxWalkSoundCnt)
        {
            SoundManager.instance.SfxPlaySound2(3, transform.position);//숨소리
            curWalkSoundCnt = 0;
        }
    }
    private void Run()
    ...
    private void Die()
    {
        if(GameManager.instance.curWaterReserves <= 0) //현재 시작 물보유량이 0이어서 test용
        {
            if(!isSlime)
            {
