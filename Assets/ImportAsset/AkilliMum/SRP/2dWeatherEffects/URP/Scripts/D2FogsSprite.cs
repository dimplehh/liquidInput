...
{
    [ExecuteInEditMode]
    public class D2FogsSprite : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            foreach (var mat in EffectedMaterials)
            {
                ...
                if (Curve && mat.HasProperty("_CurveTex"))
                {
                    mat.SetTexture("_CurveTex", Curve.Texture);
                    if (Curve.Texture)
                        mat.EnableKeyword("_USECURVE");
                    else
                        mat.DisableKeyword("_USECURVE");
                }
